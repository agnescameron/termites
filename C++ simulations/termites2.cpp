//a modification of the Cortois model of termite behaviour to include
//an extra chemoattractant parameter
//Agnes Cameron, 2017
...
//prints the status of the grid (termites)
void printChemoAttractant(vector<cell> cellVector, int cycles){
        int citySize = sqrt(cellVector.size());
    
    cout << "chemoattractant" << endl;
    for(int i=1; i<citySize; i++){
    cout << "- ";
    }
    cout << endl;
    for(int j = 1; j<citySize; j++){
        for(int i=1; i<citySize; i++){
            if(cellVector[i + j*citySize].chemoAttractant > 30){
                cout << "x";
            }
            else
                cout << " ";
...
//measures the average time needed to navigate from one square to another, acts as maze solver.
//Iterates between 2 randomly chosen squares, reapeats for citySize times
    ...
    bool broken = false;
        ...
        do{
            ...
            if(cellVector[randCell1].earth < threshold && cellVector[randCell2].earth < threshold && cellVector[randCell1].x < citySize && cellVector[randCell1].y < citySize && cellVector[randCell2].x < citySize && cellVector[randCell2].y < citySize && cellVector[randCell1].x > 0 && cellVector[randCell1].y > 0 && cellVector[randCell2].x > 0 && cellVector[randCell2].y > 0){
                ...
                //measures distance from each neighbouring square to the destination
                do{
                   cout << "pos is " << posi << " " << posj << " dest is " << desti << " " << destj << endl;
                    ...
                    //finds the minimum neighbouring element that is of 'street' type
                    do{
                    ...
                    int minEl = (min_element(distVector.begin(), distVector.end()) - distVector.begin());
                        ...
                        if(minEl == 0){
                             ...
                             if(cellVector[(posj-1)*citySize + posi-1].earth < threshold && posj-1>=0 && posi-1>=0 && cellVector[(posj-1)*citySize + posi-1].isDeadEnd == false){
                                 posi = posi-1;
                                 posj = posj-1;
                                 movement m {posi, posj, numSteps};
                                 moveVector.push_back(m);                                 
                    ...
                    if(numSteps > 1 && ((moveVector[numSteps].x == moveVector[numSteps-1].x && moveVector[numSteps].y == moveVector[numSteps-1].y) || (moveVector[numSteps].x == moveVector[numSteps-2].x && moveVector[numSteps].y == moveVector[numSteps-2].y))){
...
void moveTermites(vector<cell> cellVector, vector<termite> &termiteVector){
    ...
    do{
        ...
        int randDirection = rand() %8;
            ...
            //northwest
            if (randDirection == 0 && cellVector[(termitePosj+1)*sqrt(cellVector.size()) + termitePosi-1].termitePop < maxpop && 0 < termitePosj+1 && termitePosj+1 < citySize && cellVector[(termitePosj+1)*sqrt(cellVector.size()) + termitePosi-1].chemoAttractant > rand() %50){
            ...
            //north
            if (randDirection == 1 && cellVector[(termitePosj+1)*sqrt(cellVector.size()) + termitePosi].termitePop < maxpop && 0 < termitePosj+1 && termitePosj+1 < citySize && cellVector[(termitePosj+1)*sqrt(cellVector.size()) + termitePosi].chemoAttractant > rand() %50){
                if(termiteVector[a].hasEarth == true){
             ...
             //northeast
              else if (randDirection == 2 && cellVector[(termitePosj+1)*sqrt(cellVector.size()) + termitePosi+1].termitePop < maxpop && termitePosj+1 < citySize && termitePosi+1 < citySize  && cellVector[(termitePosj+1)*sqrt(cellVector.size()) + termitePosi+1].chemoAttractant > rand() %50){
                  if(termiteVector[a].hasEarth == true){
                      termiteVector[a].x = termitePosi+1;
                      termiteVector[a].y = termitePosj+1;
                  }
            ...
            //east
            else if (randDirection == 3 && cellVector[(termitePosj)*sqrt(cellVector.size()) + termitePosi+1].termitePop < maxpop && 0 < termitePosi + 1 && termitePosi + 1 < citySize  && cellVector[(termitePosj)*sqrt(cellVector.size()) + termitePosi+1].chemoAttractant > rand() %50){
                if(termiteVector[a].hasEarth == true){
                    termiteVector[a].x = termitePosi+1;
            ...
            //southeast
             else if (randDirection == 4 && cellVector[(termitePosj-1)*sqrt(cellVector.size()) + termitePosi+1].termitePop < maxpop && 0 < termitePosj-1 && termitePosj+1 < citySize && cellVector[(termitePosj-1)*sqrt(cellVector.size()) + termitePosi+1].chemoAttractant > rand() %50){
                 if(termiteVector[a].hasEarth == true){
                     termiteVector[a].x = termitePosi+1;
                     termiteVector[a].y = termitePosj-1;
                 }
            ...
            //south
            else if (randDirection == 5 && cellVector[(termitePosj-1)*sqrt(cellVector.size()) + termitePosi].termitePop < maxpop && 0 < termitePosj-1 && termitePosj-1 < citySize && cellVector[(termitePosj-1)*sqrt(cellVector.size()) + termitePosi].chemoAttractant > rand() %50){
                if(termiteVector[a].hasEarth == true){
                    termiteVector[a].x = termitePosi;
                    termiteVector[a].y = termitePosj-1;
                }
            ...
            //southwest
             else if (randDirection == 6 && cellVector[(termitePosj-1)*sqrt(cellVector.size()) + termitePosi-1].termitePop < maxpop && 0 < termitePosi - 1 && 0 < termitePosj - 1 && cellVector[(termitePosj-1)*sqrt(cellVector.size()) + termitePosi-1].chemoAttractant > rand() %50){
                 if(termiteVector[a].hasEarth == true){
                     termiteVector[a].x = termitePosi-1;
                     termiteVector[a].y = termitePosj-1;
                 }
            ...
            //west
            else if(randDirection == 7 && cellVector[(termitePosj)*sqrt(cellVector.size()) + termitePosi-1].termitePop < maxpop &&  0 < termitePosi - 1 && termitePosi - 1 < citySize && cellVector[(termitePosj)*sqrt(cellVector.size()) + termitePosi-1].chemoAttractant > rand() %50){
                if(termiteVector[a].hasEarth == true){
                    termiteVector[a].x = termitePosi-1;
                    termiteVector[a].y = termitePosj;
                }
    ...
    do{
        ...
        if(termiteVector[a].hasEarth == true){
            if ((rand() % 100) > probPlace*100 && cellVector[cellVectorPos].earth < maxheight){
                ...
                cellVector[(termitePosj+1)*sqrt(cellVector.size()) + termitePosi].chemoAttractant = cellVector[(termitePosj+1)*sqrt(cellVector.size()) + termitePosi].chemoAttractant + 20;
                cellVector[termitePosj*sqrt(cellVector.size()) + termitePosi+1].chemoAttractant = cellVector[termitePosj*sqrt(cellVector.size()) + termitePosi+1].chemoAttractant + 20;
                cellVector[(termitePosj-1)*sqrt(cellVector.size()) + termitePosi].chemoAttractant = cellVector[(termitePosj-1)*sqrt(cellVector.size()) + termitePosi].chemoAttractant + 20;
                cellVector[termitePosj*sqrt(cellVector.size()) + termitePosi-1].chemoAttractant = cellVector[termitePosj*sqrt(cellVector.size()) + termitePosi-1].chemoAttractant + 20;
            }
        }
            ...
            cellVector[cellVectorPos].earth = cellVector[cellVectorPos].earth - 1;
                            cellVector[(termitePosj+1)*sqrt(cellVector.size()) + termitePosi].chemoAttractant = cellVector[(termitePosj+1)*sqrt(cellVector.size()) + termitePosi].chemoAttractant - 10;
                cellVector[termitePosj*sqrt(cellVector.size()) + termitePosi+1].chemoAttractant = cellVector[termitePosj*sqrt(cellVector.size()) + termitePosi+1].chemoAttractant - 10;
                cellVector[(termitePosj-1)*sqrt(cellVector.size()) + termitePosi].chemoAttractant = cellVector[(termitePosj-1)*sqrt(cellVector.size()) + termitePosi].chemoAttractant - 10;
                cellVector[termitePosj*sqrt(cellVector.size()) + termitePosi-1].chemoAttractant = cellVector[termitePosj*sqrt(cellVector.size()) + termitePosi-1].chemoAttractant - 10;
            }
        }
...
int main(){
    ...
    initialiseTermites(cellVector, termiteVector);
        ...
        do{
            ...
            //finds thresholded coverage
            findCoverage(cellVector, cycles, coverage);
            system("sleep 0.2");
            
            //printChemoAttractant(cellVector, cycles);
            //system("sleep 0.1");
        ...
        for(int j = 0; j<citySize*citySize; j++){
                cellfile << cellVector[j].x << "," << cellVector[j].y << "," << cellVector[j].earth << "," << cellVector[j].chemoAttractant;
            cellfile << "\n";
        }
        cellfile.close();
